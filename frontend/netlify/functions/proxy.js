...
exports.handler = async (event) => {
  try {
    // Extract path and construct backend URL
    const path = event.path.replace("/.netlify/functions/proxy", "");
    const backendUrl = `http://144.126.253.174:8083${path}`; // Double-check port
    ...
    return {
      ...
      headers: {
        ...
        "Access-Control-Allow-Origin": "*", // Consider specifying allowed origins
    ...
    console.error("Proxy error:", err); // Log full error for debugging
    return {
      statusCode: 500,
      headers: {
