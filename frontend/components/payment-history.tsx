...
interface AffiliateEvent {
    ...
    commissionAmount?: number; // Added for completeness if present on event object
}
...
export function PaymentHistory({ events, limit, searchQuery = "", filter = "all" }: PaymentHistoryProps) {
    // Payment methods for mapping paymentMethodId (Mock data must match backend IDs)
    const paymentMethods = [
        ...
        // ------------------------------------
        // Stripe Standard Account Payout Method
        {
            id: "5",
            type: "stripe_standard",
            name: "Stripe Standard Account",
            details: "Connected",
            methodDisplay: "Stripe Payout",
        },
        // ------------------------------------
    ]
    ...
    const filteredTransactions = events
        .filter((event) => {
            ...
            if (event.event !== "commission" && event.event !== "payout" && event.event !== "conversion") return false;
            
            const isEarning = event.event === "commission" || event.event === "conversion";
            const isWithdrawal = event.event === "payout";

            if (filter === "all") return true;
            if (filter === "earnings" && isEarning) return true;
            if (filter === "withdrawals" && isWithdrawal) return true;
            return false;
        })
        .filter((event) =>
    ...
    const getDescription = (event: AffiliateEvent) => {
        if (event.event === "commission" || event.event === "conversion") {
            ...
            // Use the payment method's name in the description
    ...
    const getMethodDisplay = (event: AffiliateEvent) => {
        if (event.event === "payout") {
            const method = paymentMethods.find((m) => m.id === event.paymentMethodId);
            // Use the payment method's methodDisplay for a concise view
    ...
    return (
        <div className="space-y-4">
            {displayTransactions.map((event) => {
                // Determine the amount to display, prioritizing commissionAmount if present for earnings
                const displayAmount = event.commissionAmount ?? event.amount;
                            ...
                            <div className="text-right">
                                <p
                                    className={`font-semibold ${
                                        (event.event === "commission" || event.event === "conversion") ? "text-green-500" : (event.status?.toLowerCase() === "failed" ? "text-red-500" : "")
                                    }`}
                                >
                                    {displayAmount !== undefined ? (event.event === "payout" ? "-" : "+") : ""}$
                                    {displayAmount !== undefined ? Math.abs(displayAmount).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "0.00"}
                                </p>
                            </div>
