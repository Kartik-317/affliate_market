...
export interface AffiliateEvent {
	...
	read?: boolean;
	_id?: string;
	type?: string; // Added to capture explicit DB type like 'withdrawal'
}
...
// CORRECTED OptimizationSuggestion interface to match CampaignOptimizationProps expectation
interface OptimizationSuggestion {
	id: string;
	type: "campaign" | "keyword" | "audience" | "creative";
	title: string;
	description: string;
	impact: "high" | "medium" | "low"; // CORRECTED TYPE
	estimatedRevenue: number;
	effort: "high" | "medium" | "low"; // CORRECTED TYPE
}
...
export default function AnalyticsPage() {
	...
	const [unreadCount, setUnreadCount] = useState(0); // Add unread count for DashboardLayout
			...
			switch (event.event) {
				case "impression":
					if (typeof event.impressions === 'number') {
						// Assuming impression means a potential click opportunity, but usually impressions are separate
						// For simplicity, I'll assume clicks field is used for 'clicks' event and impressions for 'impressions'
						// Since there's no impressions field in NetworkMetrics, I'll stick to clicks and commissions
						// To avoid changing the original logic, I'll update clicks here as per original code, though this is logically suspect
				...
				case "conversion":
				case "commission":
					// Only track positive commissions/conversions for Analytics gross revenue metrics
					...
					break;
				case "payout":
					// Ignore payouts in Analytics gross revenue/metrics processing
					break;
			}
		});
	...
	// 4. Fetch initial events and set up WebSocket (Updated with accessToken)
	useEffect(() => {
		...
		// Set up WebSockets for real-time events (Updated with token send)
		...
		connectedNetworks.forEach((network) => {
			...
			websocket.onopen = () => {
				...
				if (accessToken) {
					// Send access token for WebSocket authentication
					websocket.send(JSON.stringify({ token: accessToken }));
				} else {
					console.error(`No access token available for ${network.id} WebSocket`);
					// Don't toast/redirect here, as the useEffect above already handles no token case
				}
			};
			...
			websocket.onmessage = (event) => {
				try {
					...
					const notificationEvent: AffiliateEvent = {
						...
						type: data.notification?.type, // Grab DB type
					};
					...
					setNetworkData((prevData) => {
						...
						switch (eventData.event) {
							...
							case "conversion":
							case "commission":
								// Only count positive earnings for gross revenue
								if (typeof amount === "number" && amount > 0) {
									newMetrics.revenue += amount;
									newMetrics.commissions += 1;
									newMetrics.pending += amount;
								...
								break;
							case "payout":
								// Ignore payouts for gross revenue metrics
								break;
						}
	...
	}, [accessToken, toast, router]); // Dependency added: accessToken

	// 5. Fetch optimization suggestions (Updated with accessToken)
	useEffect(() => {
		const fetchOptimizationSuggestions = async () => {
			...
			try {
				...
				const data = await response.json();
				// Casting the data from the API response is necessary here since the API returns 
				// generic strings for impact/effort, but TypeScript expects literal types.
				// This assumes the backend data is structurally correct (e.g., impact is "high", not "High").
				setOptimizationSuggestions(data.suggestions as OptimizationSuggestion[]); 
	...
	}, [accessToken, router]); // Dependency added: accessToken

	// Calculate campaign metrics (Moved useMemo higher for accessibility)
	const campaignMetrics = useMemo(() => {
		...
		events.forEach(event => {
			// Only consider positive earnings for Campaign Metrics (Gross Revenue)
			if (event.campaign && validCampaigns.includes(event.campaign) && (event.event === "commission" || event.event === "conversion")) {
				const amount = event.commissionAmount ?? event.amount ?? 0;
				if (typeof amount === 'number' && amount > 0) {
				...
				// console.warn(`Invalid campaign name found: ${event.campaign}`); // Excessive logging removed
			}
		});
		return metrics;
	}, [events]);
		...
		const calculateAnalyticsData = () => {
			...
			events.forEach((event) => {
				if (!event.date || ((event.event === "commission" || event.event === "conversion") && typeof event.commissionAmount !== 'number' && typeof event.amount !== 'number')) {
					// console.warn("Skipping invalid event:", event); // Excessive logging removed
					return;
				}
				...
				if (event.event === "commission" || event.event === "conversion") {
					...
					// Only add positive commissions/conversions to Total Revenue metrics
					if (amount > 0) {
						totalRevenue += amount;
						totalCommissions += 1;
					}

					// Period over period revenue check
			...
			// console.log(`Total Commission Amount: $${totalRevenue.toFixed(2)}, Total Commission Count: ${totalCommissions}`); // Excessive logging removed

			const revenueGrowth = previousPeriodRevenue > 0
				? ((currentPeriodRevenue - previousPeriodRevenue) / previousPeriodRevenue * 100)
				: (currentPeriodRevenue > 0 ? null : 0); // Set to null if there's revenue now but none before
	...
	// Prepare chart data for conversion analysis (Kept for use in ConversionAnalysis component)
	const conversionChartData = useMemo(() => {
		const dailyData: { [key: string]: { clicks: number; commissions: number; conversionRate: number } } = {};

		events.forEach(event => {
			// Exclude Payouts from Conversion Chart Data (Gross Commissions)
			if (event.date && (event.event === "commission" || event.event === "click" || event.event === "conversion") && event.campaign && validCampaigns.includes(event.campaign)) {
				...
				// Only count positive earnings as a commission
				const amount = event.commissionAmount ?? event.amount ?? 0;
			...
			<div className="space-y-8">
				...
				<Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
					...
					<TabsContent value="forecasting" className="space-y-6">
						{/* FIXED: Pass the accessToken prop */}
